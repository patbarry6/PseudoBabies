...
##### RUN FRANz #####   
Rn.FRANz<-function(nSim,Miss_data=T,MissingVec,Geno_Error=T,ErrorVals,Markerfile='MarkerPanels.csv',ShowProgress=FALSE,...){
  wd<-getwd()
    ...
    for (e in (1:length(ErrorVec))){
      ...
      for(MS in (1:length(MissingVec)) ){
        ...
        for (s in 1:nSim){
          ...
          # Running FRANz#
          ParSampN<-length(grep(pattern="2000 ?",x= readLines(paste(wd,"/",'Panel_uSats',markers[z,2],'_SNPs',markers[z,3],'/','Error_',ErrorVec[e],'/',"MissDat_",Pmd,"/SimNum", s,"/","FRANzSim",s,".dat",sep=""))))
          
          FRANzCmds=paste('--femrepro 1:1 --malerepro 1:1 --typingerror " ',max(Type_Error[,2]),'" --mintyped 1 --N ',ParSampN,' --n ', ParSampN,sep='') #,' -q'
          cmd<- paste("FRANz", paste(wd,"/",'Panel_uSats',markers[z,2],'_SNPs',markers[z,3],'/','Error_',ErrorVec[e],'/',paste("MissDat_",Pmd,
                                            "/SimNum", s,"/","FRANzSim",s,".dat ",sep=""),FRANzCmds,sep=""))
          ...
          sapply(1:length(files2copy),function(y) file.copy(from=file.path(files2copy)[y],to=paste(wd,"/",'Panel_uSats',markers[z,2],'_SNPs',markers[z,3],'/','Error_',ErrorVec[e],"/MissDat_",Pmd,"/","SimNum",s,sep="")))
...
#' @examples
#' Resume.Rn.FRANz()
Resume.Rn.FRANz<-function(nSim,Miss_data=T,MissingVec,Geno_Error=T,ErrorVals,Markerfile='MarkerPanels.csv',ShowProgress=FALSE,MarkerNum.Res,ErrorVec.Res,MissingNum.Res,Sim.Res){
  wd<-getwd()
  if(file.exists(Markerfile)){
    markers<-read.csv(Markerfile, stringsAsFactors = F) 
