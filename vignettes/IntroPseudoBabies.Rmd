---
title: "Introduction to PseudoBabies"
...
vignette: >
  %\VignetteIndexEntry{Introduction to PseudoBabies}
...
```{r eval = FALSE}
CompleteGenotypes(file="ACS2008_FullPanel.csv",nloci=105,StartYear = "2008")
```
...
```{r HomogeneityPvals, fig.width=6}
nsims<-10
nloci<-105
...
ggplot(data=GDres[order(as.numeric(GDres[,5])),],aes(x=Population1,y=as.numeric(as.character(Pvalue))))+
  geom_boxplot()+
  ylab("Probability")+
...
Well shoot `r GDres[order(GDres$Pvalue),][1,1] ` has a probability of `r  GDres[order(GDres$Pvalue),][1,5]` for population `r  GDres[order(GDres$Pvalue),][1,2]`. How far off are the allele frequencies really? To figure this out we can pull the allele frequency distribution from the output file:
...
# Future developments
1. Add output for programs that are SNP specific. 
